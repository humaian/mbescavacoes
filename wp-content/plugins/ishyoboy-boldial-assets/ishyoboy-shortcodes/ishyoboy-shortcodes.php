...
if ( ! class_exists( 'Ishyoboy_Shortcodes' ) ) :
	...
	class Ishyoboy_Shortcodes extends Ish_Plugin_Base {
		...
		public function __construct() {
			...
			add_action( 'ish_theme_options_after_woocommerce_options', array(&$this, 'add_theme_options_misc_options'), 11 );
		...
		public function action_frontend_scripts() {
			...
			if ( isset($ish_options) && isset($ish_options['google_maps_api_key']) && '' !== $ish_options['google_maps_api_key'] ) {
				wp_register_script( 'ish-gmaps', '//maps.googleapis.com/maps/api/js?v=3.exp&key=' . esc_attr($ish_options['google_maps_api_key']) . '&callback=initialize', array( 'jquery' ), false, true );
			} else {
				wp_register_script( 'ish-gmaps', '//maps.googleapis.com/maps/api/js?v=3.exp&callback=initialize', array( 'jquery' ), false, true );
			}
		...
		public function action_admin_scripts_init() {
			...
			wp_register_script( 'ish-custom-views', IYB_SC_PLUGIN_URI . '/assets/backend/js/composer-views-extend.js', array('vc-backend-min-js'), false, true );
