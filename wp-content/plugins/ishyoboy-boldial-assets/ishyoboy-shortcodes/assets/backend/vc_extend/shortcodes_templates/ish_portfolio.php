				...
				case 'gallery' :
					...
					if ( $images ){
						foreach ( $images as $attachment_id ) {
							if ( $first_image ){
								$popup_opener = '<a href="' . wp_get_attachment_url( $attachment_id ) . '" rel="portfolio-sc-' . $ish_sc_count .'-gallery-' . get_the_ID() . '" class="openfancybox-image">';
								$first_image = false;
							} else {
								$popup_gal .= '<a href="' . wp_get_attachment_url( $attachment_id ) . '" rel="portfolio-sc-' . $ish_sc_count .'-gallery-' . get_the_ID() . '" class="ish-popup-gal-images openfancybox-image"></a>';
							}
						}
					}
					...
					if ( '' == $popup_opener  ){
						if ( is_array( $img_full )  ){
							$popup_opener = '<a href="' .  esc_attr($img_full[0]) . '" rel="portfolio-sc-' . $ish_sc_count .'-gallery-' . get_the_ID() . '" class="openfancybox-image">';
						}
						else{
